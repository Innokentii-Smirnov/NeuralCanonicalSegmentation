...
def make_model(vocabularies: dict[str, Vocabulary], device: torch.device, max_sequence_length: int) -> MorphonologicalTransducer:
    model = MorphonologicalTransducer(
        vocabularies,
        EncoderArguments(len(vocabularies["phon"]), 150, 0.1, 400, 1, 0.1, True),
        0.1,
        DecoderArguments(vocabularies["morphon"], 150, 800, 1, 0.1),
        device,
        max_sequence_length
    )
    return model
